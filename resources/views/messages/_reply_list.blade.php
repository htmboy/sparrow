        ...
        <li class=" media" name="reply{{ $reply->id }}" id="reply{{ $reply->id }}">
            ...
            <div class="media-body">
                <div class="media-heading mt-0 mb-1 text-secondary">
                    <a href="{{ route('users.show', [$reply->user_id]) }}" title="{{ $reply->user->name?:$reply->user->username }}">
                        {{ $reply->user->name?:$reply->user->username }}
                    </a>
                    <span class="text-secondary"> • </span>
                    <span class="meta text-secondary" title="{{ $reply->created_at }}">{{ \Carbon\Carbon::parse($reply->created_at)->diffForHumans() }}</span>
